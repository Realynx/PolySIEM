...
export function focusFootprintGraph(
  ...
  const machineById = new Map(
    allMachines.map((machine) => [machine.id, machine]),
  );
  ...
  const targetFirewall = graph.firewalls.find(
    (machine) => machine.id === targetId,
  );
  const targetSwitch = graph.switches.find(
    (machine) => machine.id === targetId,
  );
  ...
  const reachability = targetFirewall
    ...
    : graph.reachability.filter(
        (edge) =>
          primaryLaneIds.has(edge.source) || primaryLaneIds.has(edge.target),
      );
  ...
  const routes = graph.routes.filter(
    (route) =>
      route.targetId === targetId || directTunnelIds.has(route.tunnelId),
  );
  ...
  const switchLinks = graph.switchLinks.filter((link) => {
    const targetLane =
      link.kind === "carriage" && primaryLaneIds.has(link.targetId);
    const targetMachineLink =
      link.kind === "uplink" && link.targetId === targetId;
  ...
  const lanes = graph.lanes
    ...
    .map((lane) => {
      ...
      const workloadPolicy = lane.workloadPolicy
        ? {
            ...lane.workloadPolicy,
            protectedCount: machines.filter((machine) => machine.workloadPolicy?.firewallEnabled).length,
            workloadCount: machines.filter((machine) => machine.kind === "vm" || machine.kind === "ct").length,
  ...
  const gateways =
    firewalls.length > 0 || tunnels.length > 0 ? [...graph.gateways] : [];
