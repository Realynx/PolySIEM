...
/**
 * GET  — sanitized connector DTOs (never a token or token hash), optionally
 *        scoped to one edge server with `?integrationId=`.
 * POST — create a connector. The response carries the one-time plaintext install
 *        token and the paste-ready one-liner; it is the ONLY place either exists.
 */
