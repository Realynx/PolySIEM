...
- **Connector** — Any far end of an Edge NAT Server's WireGuard tunnel that PolySIEM records and allocates a tunnel address for. Every kind dials out to the edge and holds the tunnel open, so nothing at home needs a public IP or an inbound port. Kinds differ only in how much of the far side PolySIEM manages.
...
- **Paste-ready peer block** — The values a manual connector's far side needs: the edge endpoint, the edge public key, the AllowedIPs for the edge peer, the tunnel address PolySIEM allocated for that far side, and the keepalive. PolySIEM shows the block and accepts the far side's PUBLIC key back; the far side always initiates, because the edge only listens.
